...
for lib in ['ppa_op_graph_classification', 'ppa_operations', 'ppa_utils', 'ppa_genotypes', 'ppa_message_passing', 'ppa_pyg_gnn_layer', 'ppa_agg_zoo', 'ppa_inits', 'deepergcn_with_hig', 'ppa_conv', 'ppa_algos', 'ppa_operations']:
...
import pyximport
# import algos
copyfile(src = '/kaggle/usr/lib/ppa_operations/ppa_operations.py', dst = '/kaggle/working/ppa_operations.py')

from ppa_operations import make_degree, make_multihop_edges
...
class NetworkGNN(nn.Module):
  ...
  def __init__(self, genotype, criterion, in_dim, out_dim, hidden_size, num_layers=3, in_dropout=0.2, out_dropout=0.2, act='relu', args=None,is_mlp=False, num_nodes=0):
    ...
    #         torch.nn.Sequential(torch.nn.Linear(hidden_size, 2 * hidden_size), torch.nn.BatchNorm1d(2 * hidden_size),
    #                             torch.nn.PReLU(), \
    #                             torch.nn.Linear(2 * hidden_size, hidden_size), torch.nn.BatchNorm1d(hidden_size), torch.nn.PReLU()))
  ...
  def forward(self, data, perturb=None):
    ...
    for i in range(0, self.num_layers):
        ...
        if len(x_list) == 0:
          ...
          # x2 = x2 + virtualnode_embedding[batch]
          h_list[i] = x2
        ...
        #           self.mlp_virtualnode_list[i](virtualnode_embedding_temp), self.dropout, training=self.training)

    x_list = self.Se_layers[i + 1](h_list)
    x = self.Fu_layers[i + 1](x_list)
    # x = self.batch_norms[i + 1](x)
    x = F.elu(x)
    x = F.dropout(x, p=self.dropout, training=self.training)

    # x = F.relu(self.last_mlp(x2))
